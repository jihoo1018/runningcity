...
//    }
     ...
     */
    @Transactional(readOnly = true)
    public RandomAvatarResponse getUserAvatarWithStats(Long userId) {
        ...
        // Step 2: 운동 통계 계산 (getMyOffice 로직 재사용!)
        List<RunSession> sessions = runSessionRepository.findAllByUserIdAndEndTimeIsNotNull(userId);
        ...
        List<RandomAvatarResponse.EquippedItemDto> equippedItemDtos =
                equippedItems.stream()
                        .map(item -> RandomAvatarResponse.EquippedItemDto.builder()
                                .itemId(item.getItemId())
                                .category(item.getCategory())
                                .subcategory(item.getSubcategory())
                                .style(item.getStyle())
    ...
    public List<RandomAvatarResponse> getFriendAvatars(Long currentUserId, Integer size) {
        ...
        if (friends.isEmpty()) {
            log.error("⚠️ [데이터 정합성 오류] 친구 ID는 있지만 User 데이터가 없음");
            return List.of();
        }

        // Step 3: 각 친구의 아바타 정보 조회 (기존 메서드 재사용!)
        List<RandomAvatarResponse> result = friends.stream()
                .map(friend -> getUserAvatarWithStats(friend.getUserId()))
    ...
    public List<RandomAvatarResponse> getRandomAvatars(Long currentUserId, int size) {
        ...
        // Step 4: 각 유저의 아바타 정보 조회 (기존 메서드 재사용!)
        List<RandomAvatarResponse> result = randomUsers.stream()
                .map(user -> getUserAvatarWithStats(user.getUserId()))
                .filter(Objects::nonNull)
                .limit(size)
                .toList();
