...
@Service
public class MonthlyRunningService {

    private final MonthlyRunSessionRepository repository;

    public MonthlyRunningService(MonthlyRunSessionRepository repository) {
        this.repository = repository;
    }

    public MonthlyRunningResponse getMonthly(String userId, int year, int month) {
        ...
        List<MonthlyRunSession> sessions = repository.findMonthlySessions(userId, start, end);

        // calendar
        LocalDate endDate = first.withDayOfMonth(first.lengthOfMonth());
        Map<Integer, Boolean> hasRecordByDay = new HashMap<>();
        for (MonthlyRunSession rs : sessions) {
            LocalDate d = rs.getStartTime().atZoneSameInstant(zone).toLocalDate();
            hasRecordByDay.put(d.getDayOfMonth(), true);
        }
        List<MonthlyRunningResponse.CalendarDay> calendarDays = new ArrayList<>();
        for (int d = 1; d <= endDate.getDayOfMonth(); d++) {
            calendarDays.add(new MonthlyRunningResponse.CalendarDay(d, hasRecordByDay.getOrDefault(d, false)));
        }
        ...
        MonthlyRunningResponse.MonthSummary summary = new MonthlyRunningResponse.MonthSummary(
        ...
        // records
        List<MonthlyRunningResponse.RunningRecordDto> records = sessions.stream()
                .map(rs -> new MonthlyRunningResponse.RunningRecordDto(
        ...
        return new MonthlyRunningResponse(
                year,
                month,
                summary,
                calendarDays,
