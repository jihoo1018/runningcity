...
-- 완전체 옷 넣는 테이블
CREATE TABLE IF NOT EXISTS user_complete_sprites (
                                                     sprite_id BIGSERIAL PRIMARY KEY,
                                                     user_id BIGINT NOT NULL UNIQUE,
                                                     sprite_name VARCHAR(100) NOT NULL,
    sprite_path VARCHAR(300) NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT fk_user_complete_sprites_user
    FOREIGN KEY (user_id)
...
CREATE TABLE IF NOT EXISTS daily_mission (
     ...
     user_id    BIGINT        NOT NULL,
     created_at       TIMESTAMPTZ   NOT NULL,
     target_km  DOUBLE PRECISION NOT NULL DEFAULT 0,
     current_km DOUBLE PRECISION NOT NULL DEFAULT 0,
     completed  BOOLEAN       NOT NULL DEFAULT FALSE,
     claimed    BOOLEAN       NOT NULL DEFAULT FALSE,
     updated_at TIMESTAMPTZ(6) NOT NULL DEFAULT NOW(),
    CONSTRAINT ck_daily_mission_nonneg
    CHECK (target_km >= 0 AND current_km >= 0),
    CONSTRAINT fk_daily_mission_user
    FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE
    );
