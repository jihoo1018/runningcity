...
LOG_FILE = "log/db_insert_hair.log"
...
def insert_hair(conn, df):
    """Hair 데이터 삽입"""
    logger.info(f"💾 {len(df)}개 Hair 아이템 삽입 시작...")
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("🚀 Hair DB Manager 시작")
    logger.info("=" * 60)
    ...
    if not csv_path.exists():
        logger.error(f"\n❌ CSV 파일 없음: {CSV_FILE}")
        logger.info("💡 먼저 'python asset_scanner_hair.py'를 실행하세요")
        exit(1)
    ...
    # 5. 검증
    cur = conn.cursor()
    cur.execute("SELECT COUNT(*) FROM boutique_items WHERE category='hair'")
    total = cur.fetchone()[0]
    logger.info(f"\n📊 DB에 저장된 Hair 아이템: {total}개")
