# ============================================================
#  🧩 asset_scanner_bodies.py
#  ────────────────────────────────────────────────
#  - bodies 카테고리만 스캔
#  - walk 폴더 기준 (run/jump은 {animation} 패턴)
...
# =============== 설정 ===============
ASSET_ROOT = Path(r"C:\Users\SSAFY\Desktop\S13P31A405\AssetsStore\spritesheets\bodies")
CSV_FILE = "output/boutique_items_bodies.csv"
LOG_FILE = "log/asset_scan_bodies.log"
...
def parse_bodies_path(png_path: Path):
    """
    bodies 경로 파싱
    예: male/walk/black.png → subcategory='male', style=None, color='black'
    """
...
def scan_bodies():
    """bodies 카테고리 스캔 (walk 폴더만)"""
    logger.info(f"🔍 Bodies 스캔 시작: {ASSET_ROOT}")
    ...
    for walk_path in walk_paths:
        ...
        for png in walk_path.glob("*.png"):
            ...
            item_data = parse_bodies_path(rel_path)
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("🚀 Bodies Asset Scanner 시작")
    logger.info("=" * 60)
    ...
    # ✅ 상점 아이템만 출력
    logger.info(f"\n🏪 상점 판매 아이템 (3 CR):")
    store_items = df[df['obtain_method'] == 'store'].sort_values('color')
    for idx, row in store_items.iterrows():
        logger.info(f"   - {row['subcategory']:15s} | {row['color']:10s} | {row['price_cr']} CR")

    # Common 가챠 아이템
    logger.info(f"\n🎰 Common 가챠 아이템 (0 CR):")
    common_gacha = df[(df['rarity'] == 'common') & (df['obtain_method'] == 'gacha')].sort_values('color')
    for idx, row in common_gacha.iterrows():
        logger.info(f"   - {row['subcategory']:15s} | {row['color']:10s}")

    logger.info("=" * 60)
    logger.info("✅ 완료! boutique_items_bodies.csv 생성됨")
    logger.info("=" * 60)