...
export default function ShowroomPage() {
  ...
  const getCurrentData = () => {
    ...
    if (tab === "friend") {
      console.log(
        `[getCurrentData] 친구 탭, currentIndex=${currentIndex}, friendList.length=${friendList.length}`,
      );
      return friendList[currentIndex];
    }
    if (tab === "global") {
      console.log(
        `[getCurrentData] 글로벌 탭, currentIndex=${currentIndex}, globalList.length=${globalList.length}`,
      );
